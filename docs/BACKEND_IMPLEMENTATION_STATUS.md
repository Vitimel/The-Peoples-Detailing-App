...
- `src/data/appDataLayer.js` now includes a disabled-by-default Supabase REST adapter contract for reading services/settings/customer-safe availability/customer-safe booking history/customer-safe booking timelines/customer profile and vehicles/access-checked booking messages/owner jobs/owner notifications/owner reports/integration status/admin snapshots/developer launch readiness and calling the safe booking, guest-claim, owner closeout, owner operation, customer lifecycle, developer admin, and app-role RPCs.
