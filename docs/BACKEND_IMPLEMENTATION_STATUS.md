...
- `supabase/migrations/20260702193000_owner_notification_read_rpc.sql` adds a role-gated owner notification inbox RPC so Dane can read SMS-placeholder notifications, action-required state, and safe booking context without sending live SMS or exposing payment/app-fee internals.
- `supabase/migrations/20260702194000_customer_profile_vehicle_rpc.sql` adds signed-in customer profile and saved-vehicle RPCs so repeat customers can load/update their profile and manage their own vehicles without raw table access.
- `supabase/migrations/20260702195000_owner_reports_read_rpc.sql` adds a role-gated owner report snapshot RPC for booking totals, cash balance due, card fee, hidden app-fee ledger, SMS estimates, and BrandNew net estimates without exposing Stripe IDs or moving money.
- `supabase/migrations/20260702200000_owner_closeout_rpc.sql` adds a role-gated owner closeout RPC for adjustments, cash collected, remaining balance, refund-needed review, and completed job status without Stripe captures/refunds or money movement.
- `supabase/migrations/20260702201000_booking_timeline_read_rpc.sql` adds a safe booking timeline RPC so owners, claimed customers, and guest claim-token users can read booking status history without exposing raw auth IDs, claim-token fields, app-fee ledger rows, payment placeholders, or SMS queue internals.
- `supabase/migrations/20260702202000_guest_claim_audit_rpc.sql` hardens the guest-to-profile claim flow so saving a guest booking attaches prior messages, preserves profile/vehicle data, sets a default vehicle when appropriate, and writes a `guest_booking_claimed` timeline event.
...
- `src/data/appDataLayer.js` now includes a disabled-by-default Supabase REST adapter contract for reading services/settings/customer-safe availability/customer-safe checkout quotes/customer-safe booking history/customer-safe booking timelines/customer profile and vehicles/access-checked booking messages/owner jobs/owner notifications/owner reports/integration status/admin snapshots/developer launch readiness and calling the safe booking, guest-claim, owner closeout, owner operation, customer lifecycle, developer admin, and app-role RPCs.
